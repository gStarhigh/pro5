...
| Contact form |  I tried to get gmail to work with django core mail, but I could not get it to work. In the future I want the admin to be able to respond to the users ticket in a chat-like manner and the user should get a notification in the navbar. Now the admin has the ability to respond using the Admin panel at least.  |
| Notifications to the user | In the future I would like for the Navbar to contain a "bell" or the like, to indicate with numbers, when something has been updated or happened that is connected to the user, such as a like, comment or ticket update. |
